...
import { FolderKanban, CheckSquare, Activity, Briefcase } from 'lucide-react';

export const PMLayout: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');

  const navItems = [
    { to: '/pm', label: 'My Managed Projects', icon: <FolderKanban className="h-4 w-4" />, end: true },
    { to: '/pm/board', label: 'Team Kanban Board', icon: <CheckSquare className="h-4 w-4" /> },
    { to: '/pm/activity', label: "Team Activity Feed", icon: <Activity className="h-4 w-4" /> },
  ];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <div className="flex flex-1">
        {/* Desktop PM Sidebar: Flush with page background, border-r hairline */}
        <aside className="hidden w-64 shrink-0 border-r border-border bg-background p-4 md:block">
          <div className="mb-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-secondary border border-border text-foreground">
            ...
            <span className="text-xs font-bold uppercase tracking-wider">PM Workspace</span>
          </div>
