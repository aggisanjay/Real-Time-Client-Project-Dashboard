...
export const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    { to: '/admin', label: 'Executive Overview', icon: <LayoutDashboard className="h-4 w-4" />, end: true },
    { to: '/admin/projects', label: 'Client Projects', icon: <FolderKanban className="h-4 w-4" /> },
    { to: '/admin/tasks', label: 'Master Task Board', icon: <CheckSquare className="h-4 w-4" /> },
    { to: '/admin/clients', label: 'Client Directory', icon: <Building2 className="h-4 w-4" /> },
    { to: '/admin/users', label: 'Team & RBAC Roles', icon: <Users className="h-4 w-4" /> },
    { to: '/admin/activity', label: 'Global Live Feed', icon: <Activity className="h-4 w-4" /> },
  ];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <div className="flex flex-1">
        {/* Desktop Admin Sidebar: Flush with page background, border-r hairline */}
