...
export const PMTeamBoardPage: React.FC = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      console.error('Failed to fetch PM tasks:', err);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Assign tasks to developers, verify completed review items, and track live status.
          </p>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary/90 transition-colors self-start sm:self-auto disabled:opacity-50"
        >
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl backdrop-blur-xl">
            <h2 className="text-base font-bold text-foreground mb-4">Assign Task to Team Member</h2>
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Build API Endpoint for Order Search"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Technical instructions or scope..."
                  className="w-full rounded-xl border border-border bg-background/50 px-3.5 py-2 text-xs text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
                />
              </div>
